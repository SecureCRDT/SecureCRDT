...
import static pt.uporto.dcc.securecrdt.util.Standards.EVERGROWINGSET_PROTOCOL_NAME;

@Getter
@Setter
public class EverGrowingSetCrdt {

    private final CrdtPlayer player;

    private ArrayList<Integer> list;

    public EverGrowingSetCrdt(CrdtPlayer player){
        this.player = player;
    ...
    public void handleMessage(int type, IntProtocolMessage req) throws IOException {
        ...
        switch (type) {
            ...
            case 1: {
                ...
                this.list.add(data.getShare());

                if (req.getProtocol().equals(EVERGROWINGSET_PROTOCOL_NAME + ".merge")) break;

                DataOutputStream out = clientMessageHandler.getOutStream();
                SetPlayerState state = new SetPlayerState(list, -1);
                IntProtocolMessage response = new IntProtocolMessage(EVERGROWINGSET_PROTOCOL_NAME + ".update", state.serialize());
            ...
            case 2: {
                ...
                IntProtocolMessage response = new IntProtocolMessage(EVERGROWINGSET_PROTOCOL_NAME + ".query", state.serialize());
                out.writeInt(response.serialize().length);
                out.write(response.serialize());
                out.flush();
            ...
            case 3: {
                ...
                SetPlayerState state = new SetPlayerState(list, -1);
                IntProtocolMessage response = new IntProtocolMessage(EVERGROWINGSET_PROTOCOL_NAME + ".propagate", state.serialize());
                out.writeInt(response.serialize().length);
                out.write(response.serialize());
                out.flush();
                //System.out.println("Returned current state to the controller");
            ...
            case 4: {
                ...
                for(int v : incomingList) {
                    ...
                    IntProtocolMessage message = new IntProtocolMessage(EVERGROWINGSET_PROTOCOL_NAME + ".merge", messageData.serialize());
                    this.handleMessage(1, message);
                }

                DataOutputStream out = clientMessageHandler.getOutStream();
                SetPlayerState state = new SetPlayerState(list, -1);
                IntProtocolMessage response = new IntProtocolMessage(EVERGROWINGSET_PROTOCOL_NAME + ".merge", state.serialize());
                out.writeInt(response.serialize().length);
                out.write(response.serialize());
                out.flush();
                break;
